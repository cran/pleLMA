...
Both $i$ and $k$ will denote variables and $j_i$ and $\ell_k$ will denote categories of variables $i$ and $k$, respectively; however, to keep the notation simpler the subscripts on categories will be suppressed. The index $m=1,\ldots, M$ is used to denote latent dimensions or traits. When describing the algorithm, $n$ will be used to index for subjects (individuals, cases, etc), where $n=1,\ldots, N$. The index $n$ is suppressed in the presentation of the model. Let $\mathbf{Y}$ be an $(I\times 1)$ vector of random categorical variables and $\mathbf{y}= (y_1, \ldots, y_I)'$ be it's realization where $y_i=j$. The most general LMA for the probability that $\mathbf{Y}=\mathbf{y}$ is
...
$$ P(Y_{in}=j|\mathbf{y_{-i,n}})  =  \frac{\exp (\lambda_{ij} + \nu_{ijm} \sum_{k\ne i}\sum_{m'} \sigma_{mm'}\nu_{k\ell(n) m'})} { \sum_h \exp(\lambda_{ih} + \nu_{ihm} \sum_{k\ne i}\sum_{m'} \sigma_{mm'}\nu_{k\ell(n) m'})} \hspace{1in}  \\ 
 ...
 =  \frac{\exp (\lambda_{ij} + \sum_{m'}\sigma_{mm'}\ddot{\theta}_{ijm'n})} { \sum_h \exp(\lambda_{ih} + \sum_{m'}\sigma_{mm'}\ddot{\theta}_{ijm'n})}, \qquad (2)$$
